...
const locations: MarkersProps[] = [
	{
		...
		matching_types: [MatcherTypes.FEATURED, MatcherTypes.RECOMMENDED, MatcherTypes.DEFAULT],
	},
	{
...
export default function HomePage() {
	const [selectedLocation, setSelectedLocation] = useState<MarkersProps | null>(null);
	const [filteredLocations, setFilteredLocations] = useState<MarkersProps[]>(locations);
	const [selectedMatcher, setSelectedMatcher] = useState<MatcherTypes>(MatcherTypes.DEFAULT);

	const handleLocationSelect = (location: MarkersProps) => {
		setSelectedLocation((prev) => (prev?.key === location.key ? null : location));
	};

	const handleFilterByMatching = (matching: MatcherTypes) => {
		// Always fall back to DEFAULT when clicking the active filter
    const newFilter = selectedMatcher === matching ? MatcherTypes.DEFAULT : matching;
	...
	return (
		<React.Fragment>
			...
			<section className="min-h-screen">
				<SearchAndFilter handleFilterByMatching={handleFilterByMatching} selectedMatcher={selectedMatcher} />
				<MapContainer locations={filteredLocations} selectedLocation={selectedLocation} onLocationSelect={handleLocationSelect} />
			</section>
		</React.Fragment>
	);
}
